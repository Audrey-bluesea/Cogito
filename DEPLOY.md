# cozymemo 部署指南（Cloudflare Pages · 固定域名）

目标：把 cozymemo 部署到 Cloudflare Pages，拿到一个**永远不变**的地址
（如 `cozymemo.pages.dev`），以后改代码 `git push` 即自动更新，
手机刷新就是新版，**再也不用迁移数据**。

当前本地仓库状态：已 `git init` 并提交 v54（含手撕卡修复、做旧牛皮纸噪点、
各主题彩色底恢复）。`sw.js` 缓存版本 `cozymemo-v54`。
...
- Repository name 填 `cozymemo`，选 **Public**（私有也行，但 Pages 免费需 public 或 Pro）
...
```bash
cd /Users/leeshukyuen/WorkBuddy/2026-07-31-23-01-57/cozymemo
git remote add origin https://github.com/<你的用户名>/cozymemo.git
git branch -M main
git push -u origin main
```

> 如果装了 GitHub CLI（`gh`），也可：`gh repo create cozymemo --public --source=. --push`

### 第 3 步 · Cloudflare Pages 关联部署
- 打开 https://dash.cloudflare.com/ → 左侧 **Workers & Pages** → **Create** → **Pages** → **Connect to Git**
- 授权并选择刚才的 `cozymemo` 仓库
- 构建设置：
  - Framework preset：**None**
  - Build command：**留空**
  - Build output directory：**`/`（仓库根目录，index.html 在这）**
- 点 **Save and Deploy**
- 约 1 分钟后得到固定地址，形如 `https://cozymemo.pages.dev`

### 第 4 步 · 最后一次数据迁移（一次性）
新地址是**新 origin**，旧数据在 `868d63f3…bj3`，需搬过来：
1. 打开旧地址 `https://868d63f37fd24c55b0ae1abdb629b9e3.bj3.agentos-app.net`
   → 设置 → 导出备份 → **复制文本**
2. 打开新地址 `https://cozymemo.pages.dev`
   → 设置 → 导入恢复 → 粘贴 → **从文本导入** → 确认
3. 主题会一并还原。旧地址数据原样保留，可随时回退。

### 第 5 步 · 重新装到主屏
新地址数据齐全后，从 `cozymemo.pages.dev` **添加到主屏幕**。
